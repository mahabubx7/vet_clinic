...
FROM
  (SELECT count(a.*) as num,
   ...
   FROM animals a
   JOIN owners o ON a.owner_id = o.id
   GROUP BY a.owner_id) as ao
JOIN owners o2 ON ao.owner_id = o2.id
WHERE ao.num =
    (select max(max_animals.num)
     FROM
       (SELECT count(a.*) as num,
               a.owner_id
        FROM animals a
...
WHERE vets.name = 'Stephanie Mendez'
  AND v.date_of_visit BETWEEN '4-1-2020' AND '8-30-2020';


SELECT animals.name
FROM
  (SELECT count (*) as num_of_visit,
                animal_id
   FROM visits
   GROUP BY animal_id
